...
function show(req, res) {
  ...
  return res.json({ nome: "Saque 1" });
}

function list(request, response) {
  connection.query('SELECT * FROM saque', function (err, resultado){
    if(err) {
...
function create(request, response) {
  ...
  connection.query('INSERT INTO saque (valor, data_hora, conta_id) VALUES (?, ?, ?)', [valor, data_hora, conta_id], function (err, resultado){
    ...
    return response.json({valor, data_hora, conta_id, id_saq: resultado.insertId});
...
function update(req, res) {
   ...
   let validacao = new Validator(req.body, regras);
    ...
    {
        connection.query('UPDATE saque SET valor = ?, data_hora = ?, conta_id = ? WHERE id_saq = ?;',[valor, data_hora, conta_id, codigo], function (err, result) {
            if (err) {
...
function destroy(req, res) {
  const codigo = req.params.codigo;
  connection.query('DELETE FROM saque WHERE id_saq = ?;',[codigo], function (err, result) {
      if (err) {
          return res.json({erro: err.message})
      }
